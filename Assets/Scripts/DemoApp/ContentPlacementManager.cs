...
{
    [RequireComponent(typeof(AROManagerGlobal))]
    public class ContentPlacementManager : MonoBehaviour
    {
        ...
        private AROManagerGlobal m_AROManager;
        ...
        void Start()
        {
            ...
            m_AROManager = GetComponent<AROManagerGlobal>();
        ...
        public void StartOnServerLocalizer()
        {
            ...
            if (mapIds.Length > 0)
            {
                ...
                foreach (SDKMapId mapId in mapIds)
                {
                    ...
                    {
                        ARSpace.RegisterSpace(m_ARSpace.transform, m_ARMap, m_ARMap.transform.localPosition, m_ARMap.transform.localRotation, m_ARMap.transform.localScale);
                    }
                }
        ...
        public void StopOnServerLocalizer()
        {
            ...
            foreach (SDKMapId mapId in m_Sdk.Localizer.mapIds)
            {
                if (ARSpace.mapIdToMap.ContainsKey(mapId.id))
                {
                    ARSpace.UnregisterSpace(m_ARSpace.transform, mapId.id);
                }
            }
        }
