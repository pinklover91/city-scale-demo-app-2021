...
namespace Immersal.AR
{
    ...
    public class ARSpace : MonoBehaviour
    {
		...
		public static ARMap LoadAndInstantiateARMap(Transform tr, SDKJob map, byte[] mapData, ARMap.RenderMode renderMode, Color pointCloudColor)
		{
			GameObject go = new GameObject(string.Format("AR Map {0}", map.id));
			go.transform.SetParent(tr);

			ARMap arMap = go.AddComponent<ARMap>();
			arMap.renderMode = renderMode;
			arMap.color = pointCloudColor;
			arMap.InitMesh();
